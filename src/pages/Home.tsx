...
export default function Home() {
  ...
  const showSingleCtaHero = isDesktop && flags['eh-desk-hero-single-cta-desktop']

  function trackHeroCta(ctaLabel: string, destination: string) {
    trackMetric(STORE_METRIC_EVENTS.heroCtaClick, {
      cta_label: ctaLabel,
      destination,
      experiment_key: 'eh-desk-hero-single-cta-desktop',
      variation: showSingleCtaHero ? 'variant_b' : 'control',
    })
  }
    ...
    <div>
      ...
      {heroVariant === 'minimal' ? (
        <section className="bg-white border-b border-gray-100 py-16 px-4 text-center">
          ...
          <Link
            to="/flags/american"
            onClick={() => trackHeroCta('minimal_shop_all_flags', '/flags/american')}
            className="inline-block bg-[#B22234] text-white font-semibold px-8 py-3 rounded-lg hover:bg-red-700 transition-colors"
          >
            Shop All Flags
          </Link>
        </section>
        ...
        <section className="bg-gray-950 text-white py-24 px-4 text-center">
          <div className="max-w-3xl mx-auto">
            ...
            <Link
              to="/flags/american"
              onClick={() => trackHeroCta('dark_shop_now', '/flags/american')}
              className="inline-block bg-[#C9A027] text-gray-950 font-bold px-10 py-4 rounded-lg hover:bg-yellow-500 transition-colors text-lg"
            >
              Shop Now →
            </Link>
          </div>
          ...
          <div className="relative flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28 flex flex-col items-center justify-center text-center">
            ...
            {showSingleCtaHero ? (
              <div className="flex flex-col items-center gap-3">
                <Link
                  to="/flags/american"
                  onClick={() => trackHeroCta('shop_all_flags', '/flags/american')}
                  className="bg-[#B22234] text-white font-bold px-12 py-4 rounded-lg hover:bg-red-700 transition-colors shadow-lg text-lg min-w-64"
                >
                  Shop All Flags
                </Link>
                <Link
                  to="/flags/american"
                  onClick={() => trackHeroCta('american_flags_link', '/flags/american')}
              ...
              <div className="flex gap-4 flex-wrap justify-center">
                <Link
                  to="/flags/american"
                  onClick={() => trackHeroCta('shop_american_flags', '/flags/american')}
                  className="bg-[#B22234] text-white font-semibold px-8 py-3 rounded-lg hover:bg-red-700 transition-colors shadow-lg"
                >
                  Shop American Flags
                </Link>
                <Link
                  to="/flags/nautical"
                  onClick={() => trackHeroCta('browse_all_categories', '/flags/nautical')}
                  className="bg-white/15 backdrop-blur-sm text-white font-semibold px-8 py-3 rounded-lg hover:bg-white/25 transition-colors border border-white/20"
                >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {categoryCards.map(({ slug, emoji, bg }) => (
            <Link
              key={slug}
              to={`/flags/${slug}`}
              className={`bg-gradient-to-br ${bg} text-white rounded-xl py-2.5 px-3 flex flex-col justify-between min-h-[104px] shadow-sm hover:shadow-md transition-shadow`}
            >
              <span className="text-3xl">{emoji}</span>
              <div>
                <h3 className="font-semibold text-sm">{categoryMeta[slug].label}</h3>
                <p className="text-xs text-white/80 line-clamp-2">{categoryMeta[slug].description}</p>
              </div>
            </Link>
          ))}
        </div>
