...
const HERO_CATEGORY_MATCH_FLAG_KEY = 'eh-desktop-hero-category-match-desktop'
...
export default function Home() {
  ...
  const heroSecondaryCtaMatchesCategoryPromise = flags[HERO_CATEGORY_MATCH_FLAG_KEY]
  const heroSecondaryCtaDestination = heroSecondaryCtaMatchesCategoryPromise ? '#shop-by-category' : '/flags/nautical'
  const heroSecondaryCtaArm = heroSecondaryCtaMatchesCategoryPromise ? 'eh-arm-1' : 'eh-arm-0'

  function handleHeroSecondaryCtaClick() {
    trackMetric(STORE_METRIC_EVENTS.heroSecondaryCtaClick, {
      flagKey: HERO_CATEGORY_MATCH_FLAG_KEY,
      variation: heroSecondaryCtaArm,
      destination: heroSecondaryCtaDestination,
    })
  }
  ...
  return (
    <div>
        ...
        <section className="relative text-white overflow-hidden min-h-[min(85vh,640px)] flex flex-col">
          ...
          <div className="relative flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28 flex flex-col items-center justify-center text-center">
            ...
            <div className="flex gap-4 flex-wrap justify-center">
              ...
              <Link
                to={heroSecondaryCtaDestination}
      ...
      {/* Category cards */}
      <section id="shop-by-category" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 scroll-mt-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {categoryCards.map(({ slug, emoji, bg }) => (
            <Link
              key={slug}
              to={`/flags/${slug}`}
              className={`bg-gradient-to-br ${bg} text-white rounded-xl py-2.5 px-3 flex flex-col justify-between min-h-[132px] shadow-sm hover:shadow-md transition-all duration-200 hover:-translate-y-0.5`}
            >
              <span className="text-3xl mb-3">{emoji}</span>
              <div>
                <h3 className="font-bold text-lg leading-tight">{categoryMeta[slug].label}</h3>
                <p className="text-xs text-white/80 mt-1 line-clamp-2">{categoryMeta[slug].description}</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="bg-gray-50 py-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link to="/flags/state" className="text-sm font-medium text-[#B22234] hover:underline">
              Explore more →
            </Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
