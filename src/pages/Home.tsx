...
import { DESKTOP_BESTSELLER_ATC_FLAG_KEY, useLDFlags } from '../hooks/useLDFlags'
...
export default function Home() {
  ...
  const showPersistentBestSellerAtc = flags[DESKTOP_BESTSELLER_ATC_FLAG_KEY]
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {categoryCards.map(({ slug, emoji, bg }) => (
            <Link
              key={slug}
              to={`/flags/${slug}`}
              className={`bg-gradient-to-br ${bg} text-white rounded-xl py-2.5 px-3 flex flex-col items-center justify-center min-h-[8rem] shadow-sm hover:shadow-md transition-shadow`}
            >
              <span className="text-4xl mb-2">{emoji}</span>
              <span className="font-semibold text-center">{categoryMeta[slug]?.label}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-14">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500">Customer favorites from our most popular collections.</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
          {bestSellerProducts.map(product => (
            <ProductCard
              key={product.id}
              product={product}
              addToCartVisibleOverride={showPersistentBestSellerAtc}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500">Fresh additions to the FlagShip catalog.</p>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
