    ...
    <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {categoryCards.map(({ slug, emoji, bg }) => (
            <Link
              key={slug}
              to={`/flags/${slug}`}
              className={`bg-gradient-to-br ${bg} text-white rounded-xl py-8 px-5 flex flex-col items-center text-center hover:scale-[1.02] transition-transform shadow-sm`}
            >
              <span className="text-4xl mb-3">{emoji}</span>
              <span className="font-semibold">{categoryMeta[slug].label}</span>
              <span className="text-xs text-white/75 mt-1">{categoryMeta[slug].description}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-14">
        <div className="flex items-end justify-between mb-6">
          ...
          <Link to="/flags/american" className="text-sm font-medium text-[#B22234] hover:underline">Shop all →</Link>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
          {bestSellerProducts.map(p => <ProductCard key={p.id} product={p} source="homepage" />)}
        </div>
      </section>
      ...
      <section className="bg-gray-50 py-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between mb-6">
            ...
            <Link to="/flags/specialty" className="text-sm font-medium text-[#B22234] hover:underline">View more →</Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
            {newArrivals.map(p => <ProductCard key={p.id} product={p} source="homepage" />)}
          </div>
        </div>
      </section>
    </div>
