...
export default function Home() {
  ...
  const showHomepageDesktopAtc = flags['eh-desk-add-to-cart-always-visible-desktop']
    ...
    <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {categoryCards.map(({ slug, emoji, bg }) => (
            <Link
              key={slug}
              to={`/flags/${slug}`}
              className={`bg-gradient-to-br ${bg} text-white rounded-xl py-2.5 px-3 flex flex-col items-center justify-center min-h-[88px] hover:scale-[1.02] transition-transform shadow-sm`}
            >
              <span className="text-3xl mb-1">{emoji}</span>
              <span className="font-semibold text-sm text-center">{categoryMeta[slug]?.label}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-14">
        <div className="flex items-end justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Customer favorites from our most-shopped categories.</p>
          </div>
          <Link to="/flags/american" className="hidden sm:inline text-sm text-[#B22234] hover:underline">
            Shop more →
          </Link>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
          {bestSellerProducts.map(p => (
            <ProductCard
              key={p.id}
              product={p}
              trackingContext="homepage-best-sellers"
              forceAddToCartVisible={showHomepageDesktopAtc}
            />
          ))}
        </div>
      </section>
      ...
      <section className="bg-gray-50 py-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
            {newArrivals.map(p => (
              <ProductCard key={p.id} product={p} trackingContext="homepage-new-arrivals" />
            ))}
          </div>
        </div>
      </section>
