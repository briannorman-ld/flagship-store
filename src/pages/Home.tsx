...
export default function Home() {
  ...
  const browseAllCategoriesDestination = flags['eh-desktop-nav-shop-all-destination-desktop']
    ? '/#shop-by-category'
    : '/flags/nautical'
    ...
    <div>
        ...
        <section className="relative text-white overflow-hidden min-h-[min(85vh,640px)] flex flex-col">
          ...
          <div className="relative flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28 flex flex-col items-center justify-center text-center">
            ...
            <div className="flex gap-4 flex-wrap justify-center">
              ...
              <Link to={browseAllCategoriesDestination} className="bg-white/15 backdrop-blur-sm text-white font-semibold px-8 py-3 rounded-lg hover:bg-white/25 transition-colors border border-white/20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {categoryCards.map(({ slug, emoji, bg }) => (
            <Link
              key={slug}
              to={`/flags/${slug}`}
              className={`bg-gradient-to-br ${bg} text-white rounded-xl py-2.5 px-3 flex flex-col justify-between min-h-[108px] hover:scale-[1.02] transition-transform shadow-sm`}
            >
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-14">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link to="/flags/american" className="text-sm text-[#B22234] hover:underline">View all →</Link>
        </div>
      ...
      <section className="bg-gray-50 py-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link to="/search?q=new" className="text-sm text-[#B22234] hover:underline">View all →</Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
